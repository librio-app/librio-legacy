...
@section('header', trans_choice('general.categories', 2))
@section('breadcrumb')
    <li class="active">{{ trans_choice('general.categories', 2) }}</li>
@endsection

@permission('create-catalog-categories')
@section('new_button')
    <span class="new-button"><a href="{{ url('catalog/categories/create') }}" class="btn btn-success btn-sm"><span class="fa fa-plus"></span> &nbsp;{{ trans('general.add_new') }}</a></span>
@endsection
@endpermission
...
@section('content')
    ...
    <div class="box box-primary">
        <div class="box-header with-border">
            {!! Form::open(['url' => 'catalog/categories', 'role' => 'form', 'method' => 'GET']) !!}
            <div class="pull-left">
            ...
            <div class="table table-responsive">
                <table class="table table-striped table-hover" id="tbl-categories">
                    ...
                    @foreach($categories as $item)
                        <tr>
                            <td>
                                <a href="{{ url('catalog/categories/' . $item->id . '/edit') }}">
                                    {{ $item->name }}
                                </a>
                            </td>
                            <td>{{ $item->code }}</td>
                            ...
                            <td class="text-center">
                                <div class="btn-group">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        <li><a href="{{ url('catalog/categories/' . $item->id . '/edit') }}">{{ trans('general.edit') }}</a></li>
                                        @if ($item->enabled)
                                        ...
                                        @endif
                                        @permission('delete-catalog-categories')
                                        <li class="divider"></li>
                                        <li>{!! Form::deleteLink($item, 'catalog/categories') !!}</li>
                                        @endpermission
                                    </ul>
                                </div>
                            </td>
        ...
        <div class="box-footer">
            @include('partials.default.pagination', ['items' => $categories, 'type' => 'categories'])
        </div>
        <!-- /.box-footer -->
    </div>
    <!-- /.box -->
